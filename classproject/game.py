...
def character_select():
    ...
    while True:
        ...
        if s.CHARACTER in s.CHARACTERS: # if character is valid load bio
...
def survival_check():
    ...
    if health > 0:
        ...
        if ailment > 0:
            while True:
                if med >= 1:
                    takeMeds = input(f"You have {ailment} days"
...
def shop(economy):
    ...
    while True:
        ...
        if buy_item in s.SHOP:
            ...
            newQty = s.SELECTEDCHARACTER.get("Backpack").get(buy_item) + buy_qty
            s.SELECTEDCHARACTER["Backpack"][buy_item] = newQty
